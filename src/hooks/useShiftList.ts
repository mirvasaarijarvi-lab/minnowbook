...
import {
  normalizeStaffingSettings,
  type StaffingSettings,
} from "@/lib/staffing/staffingNeeds";
...
export const useStaffMembers = () => {
  const { tenantId } = useTenant();
  return useQuery({
    queryKey: ["staff_members", tenantId],
    enabled: !!tenantId,
    queryFn: async () => {
      const { data, error } = await sb
        .from("staff_members")
...
export const useStaffContacts = (enabled: boolean) => {
  const { tenantId } = useTenant();
  return useQuery({
    queryKey: ["staff_contacts", tenantId],
    enabled: !!tenantId && enabled,
    queryFn: async () => {
      const { data, error } = await sb
        .from("staff_member_contacts")
        .select("staff_member_id,email,phone")
        .eq("tenant_id", tenantId);
      if (error) throw error;
...
export const useShiftPeriods = () => {
  const { tenantId } = useTenant();
  return useQuery({
    queryKey: ["shift_periods", tenantId],
    enabled: !!tenantId,
    queryFn: async () => {
      const { data, error } = await sb
        .from("shift_periods")
...
export const usePeriodData = (periodId: string | null) => {
  const { tenantId } = useTenant();
  return useQuery({
    queryKey: ["shift_period_data", tenantId, periodId],
...
export async function fetchShiftsOnDate(
  tenantId: string,
  date: string,
  ...
  let q = sb
    ...
    .eq("date", date);
  if (siteId)
    q = q.or(siteScopeFilter(siteId), { referencedTable: SITE_SCOPE_TABLE });
  const { data, error } = await q;
  if (error) throw error;
  return data as Pick<
    ShiftRow,
...
export const useShiftsOnDate = (date: string, siteId: string | null = null) => {
  ...
  return useQuery({
    ...
    enabled: !!tenantId,
    queryFn: () => fetchShiftsOnDate(tenantId!, date, siteId),
  });
};
...
export const useStaffingSettings = () => {
  ...
  const q = useQuery({
    queryKey: ["staffing_settings", tenantId],
    enabled: !!tenantId,
  ...
  const save = useMutation({
    mutationFn: async (s: StaffingSettings) => {
      const { error } = await sb
        .from("staffing_settings")
        .upsert({ tenant_id: tenantId, settings: s });
      if (error) throw error;
    },
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["staffing_settings", tenantId] }),
  });
...
export const useShiftHistory = (periodId: string | null, enabled: boolean) => {
  const { tenantId } = useTenant();
  return useQuery({
    queryKey: ["shift_change_log", tenantId, periodId],
  ...
  return {
    ...
    saveMember: useMutation({
      mutationFn: async ({
        ...
        if (contact) {
          const { error } = await sb
            .from("staff_member_contacts")
            .upsert({ staff_member_id: id, tenant_id: t, ...contact });
          if (error) throw error;
        }
      },
      onSuccess: inv,
    ...
    setShift: useMutation({
      mutationFn: async (p: {
        ...
        if (!p.value) {
          ...
          const hasActual =
            p.existing.actual_start_time ||
            p.existing.actual_end_time ||
            p.existing.actual_note;
          const { error } = hasActual
            ? await sb
                .from("shifts")
                .update({ start_time: null, end_time: null, code: null })
                .eq("id", p.existing.id)
                .eq("tenant_id", t)
            : await sb
                .from("shifts")
    ...
    setActual: useMutation({
      mutationFn: async (p: {
        ...
        const { error } = await sb
          .from("shifts")
          .update({ actual_start_time: p.start, actual_end_time: p.end })
          .eq("id", p.existing.id)
          .eq("tenant_id", need());
        if (error) throw error;
      },
