  ...
  useEffect(() => {
    const {
      ...
      if (event === "SIGNED_OUT") {
        ...
        const tokenWasStale = prevExpiresAt !== null && prevExpiresAt <= nowSec;
        ...
        if (reason) {
          ...
          serverError &&
          /bad[_ ]?jwt|invalid[_ ]?jwt/i.test(serverError)
        ) {
    ...
    supabase.auth
      ...
      .catch((err) => {
        ...
        console.warn(
          "[AuthContext] initial getSession() failed, clearing persisted session",
          {
