...
describe("tenant access matrix", () => {
  it("covers allow, deny and inconclusive outcomes", () => {
    const verdicts = new Set<TenantAccessVerdict>(TENANT_ACCESS_MATRIX.map((c) => c.expected));
  ...
  describe.each(TENANT_ACCESS_MATRIX.map((c) => [c.label, c] as const))("%s", (_label, testCase) => {
    ...
    it("renders the outcome in the report guard table", () => {
      ...
      if (testCase.expected === "denied") {
        ...
        expect(looksClean, "denied pair rendered without any warning marker").toBe(false);
      }
    });
  });
