...
async function clientFor(role: Role): Promise<SupabaseClient> {
  switch (role) {
    case "owner":
      return signedInClient(ctx.owner);
    case "admin":
      return signedInClient(ctx.admin);
    case "staff":
      return signedInClient(ctx.staff);
    case "outsider":
      return signedInClient(ctx.outsider);
    case "anon":
      return newAnon();
  }
}
...
describe.runIf(canRun)(
  "billing + discount mutations — role matrix (live)",
  () => {
    beforeAll(async () => {
    ...
    describe("public.tenants billing column mutations", () => {
      ...
      it.each(denyRoles)(
        "role=%s cannot update ANY billing column",
        async (role) => {
      ...
      it.each(["anon", "outsider"] as Role[])(
        ...
        async (role) => {
          ...
          try {
            ...
            expect(blocked, `[${role}] SELECT must not return the row`).toBe(
              true,
            );
          } finally {
            if (role !== "anon") await client.auth.signOut();
      ...
      it.each(["anon", "outsider", "staff"] as Role[])(
        ...
        async (role) => {
          ...
          const { data: target, error: seedErr } = await ctx.service
            ...
            .single();
          if (seedErr || !target)
            throw seedErr ?? new Error("target seed failed");
