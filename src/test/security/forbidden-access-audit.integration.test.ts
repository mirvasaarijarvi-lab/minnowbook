...
describe.runIf(canRunFully)(
  ...
  () => {
    ...
    it("persists a row with action='forbidden_access', the caller's user_id, and a server timestamp", async () => {
      ...
      const { data: rows, error: readErr } = await admin
        .from("audit_log")
        .select(
          "id, action, user_id, tenant_id, table_name, new_data, created_at",
        )
    ...
    }, 30_000); // headroom for cold starts on the edge runtime without flaking. // Network round trips: function invoke + admin read. 30s leaves

    it("ignores any client-supplied user_id and attributes the row to the JWT subject", async () => {
      const tenantA = fixture.a!;
