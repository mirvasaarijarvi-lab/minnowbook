...
describe.runIf(canRun)(
  "resource_images anon SELECT cannot leak across tenants when scoped by tenant_id",
  () => {
    ...
    async function seedTenant(label: string): Promise<SeededTenant> {
      ...
      const { data: tenant, error: tErr } = await service
        ...
        .single();
      if (tErr || !tenant)
        throw tErr ?? new Error(`tenant insert failed for ${label}`);
...
describe.skipIf(canRun)(
  "resource_images anon cross-tenant scoping (skipped: missing live creds)",
  () => {
