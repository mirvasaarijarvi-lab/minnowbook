...
describe.runIf(canRun)(
  "resource_images anon SELECT stays correctly filtered under pagination + ordering",
  () => {
    ...
    beforeAll(async () => {
      ...
      for (const spec of SEED_PLAN) {
        const { data: res, error: rErr } = await service
          ...
          .single();
        if (rErr || !res)
          throw rErr ?? new Error(`resource insert failed for ${spec.label}`);
        ...
        const { data: img, error: iErr } = await service
          ...
          .single();
        if (iErr || !img)
          throw iErr ?? new Error(`image insert failed for ${spec.label}`);
    ...
    it("ascending order by sort_order returns visible rows in order and skips hidden ones entirely", async () => {
      ...
      expect(data?.map((r) => r.sort_order)).toEqual(
        expected.map((v) => v.sortOrder),
      );
...
describe.skipIf(canRun)(
  "resource_images anon pagination gating (skipped: missing live creds)",
  () => {
