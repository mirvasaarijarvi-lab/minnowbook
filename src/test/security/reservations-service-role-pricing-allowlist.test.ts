...
describe.runIf(canRun)(
  "reservations insert trigger — service_role pricing allowlist (live)",
  () => {
    beforeAll(async () => {
    ...
    it("service role keeps server-computed pricing, discount and staff fields", async () => {
      ...
      expect(
        error,
        `service insert must succeed: ${error?.message}`,
      ).toBeNull();
    ...
    it("server-side pricing persists byte-for-byte when the allowlist matches", async () => {
      ...
      const payload = {
        ...
        check_out_date: new Date(Date.now() + 10 * 86_400_000)
          .toISOString()
          .slice(0, 10),
      ...
      expect(Number(stored!.original_price_eur)).toBeGreaterThan(
        Number(stored!.price_eur),
      );
    ...
    it("a zero-cost trusted insert keeps an explicit 0 instead of dropping to null", async () => {
      ...
      const { data, error } = await ctx.service
        ...
        .select(
          "id, price_eur, original_price_eur, discount_type, discount_value",
        )
        .single();
