...
describe.runIf(canRun)(
  "run_test_reservation_cleanup authorization (live)",
  () => {
    beforeAll(async () => {
      ...
      const { error: rErr } = await ctx.service
        .from("reservations")
        .insert(rows);
      if (rErr) throw rErr;
    ...
    afterAll(async () => {
      ...
      if (ctx.adminId) {
        await swallow(
          ctx.service.from("system_admins").delete().eq("user_id", ctx.adminId),
        );
      }
     ...
     */
    const SPOOF_PAYLOADS: Array<{
      label: string;
      args: Record<string, unknown>;
    }> = [
      {
        label: "exact cron label + wide-open pattern",
        args: {
          p_source: "cron",
          p_override_pattern: "%",
      ...
      {
        label: "service_role label",
        args: {
          p_source: "service_role",
          p_override_pattern: "%",
          p_override_cutoff: "2999-01-01",
        },
      },
      ...
      {
        label: "quote-injection p_source",
        args: {
          p_source: "cron'); DROP TABLE public.reservations; --",
          p_override_pattern: "%",
          p_override_cutoff: "2999-01-01",
        },
      },
      {
        label: "cron + targeted marker pattern",
        args: {
          p_source: "cron",
          p_override_pattern: "TEST CI cleanup-authz%",
          p_override_cutoff: "2999-01-01",
        },
      },
      {
        label: "cron + null overrides (config defaults)",
        args: {
          p_source: "cron",
          p_override_pattern: null,
          p_override_cutoff: null,
        },
      },
    ...
    const logRowsSinceStart = async (): Promise<LogRow[]> => {
      const { data, error } = await ctx.service
        .from("test_reservation_cleanup_log")
        .select(
          "id, triggered_by, trigger_source, name_pattern, deleted_count, notes, triggered_at",
        )
     ...
     */
    const expectNoAuditForActor = async (
      actorId: string | null,
      context: string,
    ) => {
      ...
      for (const row of rows) {
        if (actorId !== null) {
          expect(
            row.triggered_by,
            `${context}: audit row must not credit the rejected caller`,
          ).not.toBe(actorId);
        }
        expect(
          row.trigger_source,
    ...
    it("rejects anonymous callers for every spoofed payload shape", async () => {
      ...
      for (const { label, args } of SPOOF_PAYLOADS) {
        ...
        expect(
          error,
          `anonymous cleanup call must be rejected (${label})`,
        ).not.toBeNull();
      ...
      expect(
        await spoofLogCount(),
        "rejected anon attempts must not be logged",
      ).toBe(0);
    ...
    it("rejects a signed-in non-admin for every spoofed payload shape", async () => {
      ...
      try {
        for (const { label, args } of SPOOF_PAYLOADS) {
          ...
          expect(
            error,
            `non-admin must not bypass the admin check (${label})`,
          ).not.toBeNull();
      ...
      expect(
        await spoofLogCount(),
        "rejected non-admin attempts must not be logged",
      ).toBe(0);
    ...
    it("does not treat the PostgREST service_role JWT as the cron identity", async () => {
      ...
      for (const { label, args } of SPOOF_PAYLOADS) {
        ...
        expect(
          error,
          `service_role over PostgREST must not take the cron path (${label})`,
        ).not.toBeNull();
      }
      expect(await survivingCount()).toBe(ctx.reservationIds.length);
      expect(
        await spoofLogCount(),
        "rejected service_role attempts must not be logged",
      ).toBe(0);
    ...
    it("records unauthorized cron-like runs as rejected, never as an audited cleanup", async () => {
      ...
      expect(after.length, "rejected runs must not append any audit row").toBe(
        before.length,
      );
      expect(await survivingCount()).toBe(ctx.reservationIds.length);
    }, 240_000);
    ...
    it("allows a real system admin, and logs the run as manual even if 'cron' is claimed", async () => {
      ...
      const { data: logRow, error: logErr } = await ctx.service
        .from("test_reservation_cleanup_log")
        .select(
          "trigger_source, triggered_by, deleted_count, name_pattern, notes, deleted_rows",
        )
      ...
      expect(
        logRow?.notes ?? null,
        "an authorized run is not a skipped run",
      ).toBeNull();
      ...
      const deletedIds = (
        (logRow?.deleted_rows as Array<{ id: string }> | null) ?? []
      ).map((r) => r.id);
      ...
      expect(
        adminRows.length,
        "only the single authorized admin run may be audited",
      ).toBe(1);
      expect(
        rows.every((r) => r.trigger_source !== "cron"),
        "no spoofed cron run may be audited",
      ).toBe(true);
