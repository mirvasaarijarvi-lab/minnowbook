...
describe.runIf(canRun)(
  "is_custom_role_key_assignable_by_owner (live DB)",
  () => {
    ...
    beforeAll(async () => {
      ...
      const seeds = [
        {
          role_key: `ci_safe_${stamp}`,
          hierarchy_level: 15,
          display_name: "CI Safe",
        },
        {
          role_key: `ci_admin_boundary_${stamp}`,
          hierarchy_level: 10,
          display_name: "CI Boundary 10",
        },
        {
          role_key: `ci_elevated_${stamp}`,
          hierarchy_level: 5,
          display_name: "CI Elevated 5",
        },
        {
          role_key: `ci_just_below_${stamp}`,
          hierarchy_level: 9,
          display_name: "CI Just Below 9",
        },
        ...
        // still refuse them because of the explicit NOT IN filter.
        {
          role_key: "owner",
          hierarchy_level: 50,
          display_name: "CI Rogue Owner",
        },
        {
          role_key: "superadmin",
          hierarchy_level: 50,
          display_name: "CI Rogue Superadmin",
        },
      ];
    ...
    it("rejects the reserved 'owner' key even when a role_definitions row exists at a safe level", async () => {
      if (!seeded) throw new Error("seed missing");
      expect(await callAssignable(service, seeded.tenantId, "owner")).toBe(
        false,
      );
    });
    ...
    it("rejects the reserved 'superadmin' key even when a role_definitions row exists at a safe level", async () => {
      if (!seeded) throw new Error("seed missing");
      expect(await callAssignable(service, seeded.tenantId, "superadmin")).toBe(
        false,
      );
    });
    ...
    it("rejects a custom role_key that does not exist for the tenant", async () => {
      if (!seeded) throw new Error("seed missing");
      expect(
        await callAssignable(
          service,
          seeded.tenantId,
          `ci_not_a_real_role_${Date.now()}`,
        ),
      ).toBe(false);
    });
    ...
    it("rejects a role_key that only exists on a DIFFERENT tenant", async () => {
      ...
      const { data: other, error: otherErr } = await service
        ...
        .single();
      if (otherErr || !other)
        throw otherErr ?? new Error("other tenant insert failed");
      ...
      const { data: role, error: roleErr } = await service
        ...
        .single();
      if (roleErr || !role)
        throw roleErr ?? new Error("other role insert failed");
...
describe.skipIf(canRun)(
  "is_custom_role_key_assignable_by_owner (skipped: missing live creds)",
  () => {
