...
const futureDate = () => new Date(Date.now() + 12 * 86_400_000).toISOString().slice(0, 10);
...
describe.runIf(canRun)("reservations — pricing trust decisions are audited (live)", () => {
  beforeAll(async () => {
  ...
  it("records the kept pricing fields for a trusted service_role insert", async () => {
    ...
    expect(Number(entry.new_data?.submitted_values?.original_price_eur)).toBe(120);
  });
  ...
  it("records the scrubbed pricing fields when anonymous traffic submits them", async () => {
    ...
    for (const field of ["price_eur", "original_price_eur", "discount_type", "discount_value"]) {
  ...
  it("does not log an entry for an ordinary anonymous booking with no pricing fields", async () => {
    ...
    const { error } = await ctx.anon.from("reservations").insert(baseRow(name));
    expect(error, error?.message).toBeNull();
  ...
  it("logs a trusted insert even when no pricing fields were supplied", async () => {
    ...
    const { error } = await ctx.service.from("reservations").insert(baseRow(name));
    expect(error, error?.message).toBeNull();
  ...
  it("leaves no audit entry when the insert is rejected", async () => {
    ...
    const leaked = (data ?? []).filter(
      (r) =>
        Number(
          ((r.new_data as Record<string, unknown> | null)?.submitted_values as
