...
describe.each([
  ...
  liveIt(
    ".neq tenant_id (broad scan) cannot smuggle rows past RLS",
    async () => {
      const result = await anon
        .from(table)
        .select("*")
  ...
  liveIt(
    ".or() with multiple tenant_id branches cannot smuggle rows past RLS",
    async () => {
      const result = await anon
        .from(table)
        .select("*")
  ...
  liveIt(
    "narrow column projection (id only) cannot leak row existence",
    async () => {
  ...
  liveIt("HEAD count query cannot reveal row count", async () => {
    ...
    if (error) {
      ...
      expect(
        count ?? 0,
        `${table} HEAD count must not leak true cardinality`,
      ).toBe(0);
    }
  });
  ...
  liveIt(
    "50 parallel reads with mixed filters never surface a leak",
    async () => {
...
describe.each([
  ...
  liveIt(
    "range with eq tenant_id filter returns zero rows on every page",
    async () => {
      const PAGE_SIZE = 5;
      for (let p = 0; p < 4; p++) {
        const from = p * PAGE_SIZE;
  ...
  liveIt(
    "count:'exact' alongside paginated select must not leak cardinality",
    async () => {
      ...
      expect(count ?? 0, `${table} count must not leak true cardinality`).toBe(
        0,
      );
    },
  );
  ...
  liveIt(
    "count:'planned' returns no rows (planner estimate is global, not per-tenant)",
    async () => {
  ...
  liveIt(
    "count:'estimated' returns no rows (planner estimate is global, not per-tenant)",
    async () => {
  ...
  liveIt(
    "cursor-style .gt('created_at', epoch) + order + limit returns zero rows",
    async () => {
  ...
  liveIt(
    "cursor walk (3 hops with .gt + order + limit) never surfaces a row",
    async () => {
      ...
      for (let hop = 0; hop < 3; hop++) {
        ...
        cursor = new Date(
          Date.parse(cursor) + 365 * 24 * 3600 * 1000,
        ).toISOString();
      }
    },
    15_000,
  ...
  liveIt(
    "reverse cursor (.lt + descending order) returns zero rows",
    async () => {
  ...
  liveIt(
    "range with column projection ('id') on every page returns zero rows",
    async () => {
  ...
  liveIt(
    "range across .in([live, fake]) tenant_id filter on multiple pages returns zero rows",
    async () => {
...
liveDescribe(
  "booking_validation_log — anon cannot write or mutate cross-tenant",
  () => {
...
describe.each([
  ...
  "$table — token-scoped pagination cannot leak existence",
  ({ table, handleColumn }) => {
    ...
    liveIt(
      "range(0, 9) by handle returns zero rows for revoked/expired/fake",
      async () => {
        const shapes: Shape[] = [];
        for (const handle of [
          REVOKED_TOKEN_RES_ID,
          EXPIRED_TOKEN_RES_ID,
          FAKE_RES_ID,
        ]) {
          const result = await anon
            .from(table)
          ...
          expectNoRowsLeaked(
            result,
            `${table} range(0,9) ${handleColumn}=${handle}`,
          );
    ...
    liveIt(
      "sequential pages 0-9, 10-19, 20-29 by handle leak no row + identical shapes",
      { timeout: 30_000 },
      async () => {
        const PAGE_SIZE = 10;
        const PAGES = 3;
    ...
    liveIt(
      "deep offset range(10000, 10024) by handle returns zero rows",
      { timeout: 15_000 },
      async () => {
        const shapes: Shape[] = [];
        for (const handle of [
    ...
    liveIt(
      "HEAD count by handle never reveals row count for revoked/expired/fake",
      async () => {
    ...
    liveIt(
      "count alongside paginated rows by handle leaks neither rows nor cardinality",
      { timeout: 30_000 },
      async () => {
        const shapes: Shape[] = [];
        for (const handle of [
          ...
          if (!result.error) {
            ...
            expect(
              result.count ?? 0,
              `${table} count+range count leak ${handle}`,
            ).toBe(0);
          }
          shapes.push(shapeOf(result));
        }
    ...
    liveIt(
      "reverse-order pagination by handle cannot surface latest rows",
      { timeout: 30_000 },
      async () => {
        const shapes: Shape[] = [];
        for (const handle of [
    ...
    liveIt(
      "cursor walk (.gt created_at + handle filter) never surfaces a row",
      { timeout: 30_000 },
      async () => {
    ...
    liveIt(
      "combined tenant_id + handle pagination leaks neither row nor count",
      async () => {
        const PAGE_SIZE = 10;
        const PAGES = 2;
        const perHandle: Shape[][] = [];
    ...
    liveIt(
      ".in([revoked, expired, fake]) handle list cannot smuggle rows past RLS",
      async () => {
        const result = await anon
          .from(table)
          .select("*", { count: "exact" })
        ...
        if (!result.error) {
          expect(result.count ?? 0, `${table} in() count must not leak`).toBe(
            0,
          );
        }
      },
...
describe("cross-tenant log isolation — gating", () => {
  it("documents whether the live-mode suite ran or was skipped", () => {
    if (!liveModeAvailable) {
      console.warn(
        `[cross-tenant-log-isolation] live mode skipped: ${skipReason()}`,
      );
    }
