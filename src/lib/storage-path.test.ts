...
describe("assertSafeStorageObjectPath", () => {
  ...
  describe("rejects malicious inputs", () => {
    ...
    for (const { label, path, reason } of cases) {
      it(`rejects ${label} with reason "${reason}"`, () => {
        expect(() => assertSafeStorageObjectPath(path)).toThrowError(
          InvalidStoragePathError,
        );
        try {
    ...
    it("rejects non-string input", () => {
      expect(() => assertSafeStorageObjectPath(undefined as any)).toThrowError(
        InvalidStoragePathError,
      );
      expect(() => assertSafeStorageObjectPath(123 as any)).toThrowError(
        InvalidStoragePathError,
      );
      expect(() => assertSafeStorageObjectPath(null as any)).toThrowError(
        InvalidStoragePathError,
      );
    });
  });
  ...
  describe("structured logging", () => {
    it("emits a redacted event without echoing the raw payload", () => {
      ...
      expect(() =>
        assertSafeStorageObjectPath(malicious, { callsite: "test:cs" }),
      ).toThrow();
    ...
    it("swallows logger exceptions so rejection still propagates", () => {
      ...
      expect(() => assertSafeStorageObjectPath("../bad")).toThrowError(
        InvalidStoragePathError,
      );
    });
    ...
    describe("tenantId telemetry", () => {
      ...
      it("includes valid UUID tenantId in the event", () => {
        expect(() =>
          assertSafeStorageObjectPath("../bad", { tenantId: VALID_UUID }),
        ).toThrow();
      ...
      it("normalises UUID casing and trims whitespace", () => {
        expect(() =>
          assertSafeStorageObjectPath("../bad", {
            tenantId: `  ${VALID_UUID.toUpperCase()}  `,
          }),
        ).toThrow();
        expect(events[events.length - 1]?.tenantId).toBe(VALID_UUID);
      });
      ...
      it("drops non-UUID tenantId so PII can't leak through", () => {
        for (const bad of [
          ...
          events.length = 0;
          expect(() =>
            assertSafeStorageObjectPath("../bad", { tenantId: bad }),
          ).toThrow();
          expect(events).toHaveLength(1);
