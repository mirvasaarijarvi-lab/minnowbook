...
export type SignedUrlErrorCode =
  "invalid_path" | "forbidden" | "not_found" | "transport" | "unknown";
...
export function isSignedUrlError(err: unknown): err is SignedUrlError {
  return (
    err instanceof SignedUrlError ||
    (typeof err === "object" &&
      err !== null &&
      (err as any).name === "SignedUrlError")
  );
}
...
const FORBIDDEN_RE =
  /permission|denied|policy|not\s*allowed|unauthor|forbidden/i;
const NOT_FOUND_RE = /not\s*found|object.*missing|no such|does not exist/i;
const TRANSPORT_RE =
  /fetch failed|network|timeout|aborted?|ECONN|ENOTFOUND|EAI_AGAIN|socket hang up/i;
...
export function classifySignedUrlFailure(input: ClassifyInput): SignedUrlError {
  ...
  if (
    ...
    (thrown as any).name === "InvalidStoragePathError"
  ) {
    const msg = (thrown as Error).message || "Invalid storage path";
    return new SignedUrlError(
      "invalid_path",
      contextMessage ? `${contextMessage}: ${msg}` : msg,
      { cause: thrown },
    );
  }
  ...
  if (status === 401 || status === 403) {
    return new SignedUrlError("forbidden", renderMsg("Forbidden"), {
      httpStatus: status,
      cause: thrown ?? sdkError,
    });
  }
  if (status === 404) {
    return new SignedUrlError("not_found", renderMsg("Not found"), {
      httpStatus: status,
      cause: thrown ?? sdkError,
    });
  }
  ...
  if (thrown instanceof TypeError || TRANSPORT_RE.test(haystack)) {
    return new SignedUrlError("transport", renderMsg("Transport error"), {
      httpStatus: status,
      cause: thrown ?? sdkError,
    });
  }
  ...
  return new SignedUrlError("unknown", renderMsg("Unknown signed-URL error"), {
    httpStatus: status,
    cause: thrown ?? sdkError,
  });
}
