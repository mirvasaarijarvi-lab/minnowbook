export interface FilterableSlot { staff_member_id: string | null; role_key: string | null }

const norm = (s: string) => s.toLocaleLowerCase("fi").normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
...
export function filterShiftSlots<T extends FilterableSlot>(
  ...
  return slots.filter((s) => {
    ...
    if (!q) return true;
    const hay = norm(`${s.staff_member_id ? memberName(s.staff_member_id) ?? "" : ""} ${s.role_key ? roleName(s.role_key) ?? "" : ""}`);
