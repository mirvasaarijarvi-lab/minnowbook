...
export interface DefaultRole { key: string; en: string; fi: string; sv: string }
...
export const DEFAULT_ROLES_BY_TYPE: Record<string, DefaultRole[]> = {
  ...
  venue: [
    { key: "event_host", en: "Event host", fi: "Tapahtumaisäntä", sv: "Eventvärd" },
    { key: "waiter", en: "Waiter", fi: "Tarjoilija", sv: "Servitör" },
  ],
  hotel: [
  ...
  service: [
    { key: "service_provider", en: "Service provider", fi: "Palveluntarjoaja", sv: "Tjänsteutövare" },
...
export function defaultRolesFor(types: string[]): DefaultRole[] {
  ...
  for (const t of types) for (const r of DEFAULT_ROLES_BY_TYPE[t] ?? []) {
...
export const DEFAULT_STAFFING_SETTINGS: StaffingSettings = {
  guestsPerStaff: { restaurant: 12, venue: 20, hotel: 30, guesthouse: 30, service: 1 },
  minStaff: 1,
  hourlyCostEur: 20,
...
export function normalizeStaffingSettings(raw: unknown): StaffingSettings {
  ...
  if (r.guestsPerStaff && typeof r.guestsPerStaff === "object") {
    for (const [k, v] of Object.entries(r.guestsPerStaff)) gps[k] = clampNum(v, 1, 500, gps[k] ?? 10);
  }
  ...
  return {
    ...
    rules: {
      ...
      finnishHolidays: typeof rr.finnishHolidays === "boolean" ? rr.finnishHolidays : true,
    },
  };
}

/** Staff needed for one booking. */
export function suggestStaff(type: string, guests: number, s: StaffingSettings): number {
  if (!guests || guests <= 0) return 0;
  const per = s.guestsPerStaff[type] ?? 10;
  return Math.max(s.minStaff, Math.ceil(guests / per));
}

export interface NeedBooking { reservation_type: string; start_time: string | null; end_time: string | null; guests: number }
export interface RosterShift { start_time: string | null; end_time: string | null }

const toMin = (t: string) => { const [h, m] = t.split(":").map(Number); return h * 60 + (m || 0); };

/** Covers hour h (0..23) when the interval overlaps [h, h+1). Past-midnight ends wrap. */
const covers = (start: string | null, end: string | null, h: number, fallbackLen = 120) => {
...
export interface HourNeed { hour: number; guests: number; needed: number; rostered: number; understaffed: boolean }

/** Hourly guests, staff needed from bookings and staff on the shift list for one day. */
export function hourlyNeeds(bookings: NeedBooking[], roster: RosterShift[], s: StaffingSettings): HourNeed[] {
  const out: HourNeed[] = [];
  for (let h = 0; h < 24; h++) {
    let guests = 0, needed = 0, rostered = 0;
    for (const b of bookings) if (covers(b.start_time, b.end_time, h)) {
      guests += b.guests;
      needed += suggestStaff(b.reservation_type, b.guests, s);
    }
    for (const r of roster) if (r.end_time && covers(r.start_time, r.end_time, h)) rostered += 1;
    out.push({ hour: h, guests, needed, rostered, understaffed: needed > 0 && rostered < needed });
  }
  return out;
}
