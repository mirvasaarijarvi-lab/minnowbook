...
export const parseShiftInput = (raw: string): ParsedCell => {
  ...
  if ((SHIFT_CODES as readonly string[]).includes(s)) {
    return {
      ok: true,
      value: { start_time: null, end_time: null, code: s as ShiftCode },
    };
  }
...
export const isSundayWorkDay = (
  ...
  return (
    d.getDay() === 0 ||
    (finnishHolidays && finnishSundayWorkHolidays(d.getFullYear()).has(dateISO))
  );
};
...
export const computeRowTotals = (
  ...
  for (const { date, cell } of cells) {
    ...
    const next = iso(
      new Date(d0.getFullYear(), d0.getMonth(), d0.getDate() + 1),
    );
    for (const [off, day] of [
      [0, date],
      [1440, next],
    ] as const) {
      eve += overlap(s, e, rules.eveningStart + off, rules.eveningEnd + off);
      night += overlap(s, e, off, rules.nightEnd + off);
      if (isSundayWorkDay(day, rules.finnishHolidays !== false))
        sun += overlap(s, e, off, off + 1440);
    }
  }
