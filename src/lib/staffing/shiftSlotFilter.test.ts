...
const names: Record<string, string> = {
  a: "Helena Suonio",
  b: "Jääskeläinen Pekka",
};
const roles: Record<string, string> = {
  waiter: "Vakitarjoilija",
  chef: "Kokki",
};
const run = (o: Parameters<typeof filterShiftSlots>[1]) =>
  filterShiftSlots(
    slots,
    o,
    (i) => names[i],
...
describe("filterShiftSlots", () => {
  ...
  it("searches role name", () =>
    expect(run({ query: "tarjoil" })).toEqual([1, 3]));
  it("combines words across name and role", () =>
    expect(run({ query: "suonio vaki" })).toEqual([1]));
