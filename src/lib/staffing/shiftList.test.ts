...
describe("parseShiftInput", () => {
  it("parses ranges and codes", () => {
    expect(parseShiftInput("10-18").value).toEqual({
      start_time: "10:00",
      end_time: "18:00",
      code: null,
    });
  ...
  it("formats back", () => {
    expect(
      formatShiftCell({ start_time: "09:30", end_time: "17:00", code: null }),
    ).toBe("9:30-17");
  });
});
...
describe("totals", () => {
  it("counts hours across midnight", () => {
    expect(
      shiftMinutes({ start_time: "20:00", end_time: "02:00", code: null }),
    ).toBe(360);
  });
  it("computes evening 18-24, night 0-6, sunday and codes (MaRa TES)", () => {
    const t = computeRowTotals([
      {
        date: "2026-09-27",
        cell: { start_time: "16:00", end_time: "02:00", code: null },
      }, // Sunday → Monday
      {
        date: "2026-09-28",
        cell: { start_time: null, end_time: null, code: "X" },
      },
      {
        date: "2026-09-29",
        cell: { start_time: null, end_time: null, code: "L" },
      },
    ]);
