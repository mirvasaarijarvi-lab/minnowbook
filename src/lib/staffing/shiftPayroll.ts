...
export const buildPayroll = (
  ...
  for (const slot of slots) {
    ...
    const sum: PayrollSummaryRow = {
      ...
      nightHours: 0,
      sundayHours: 0,
      codes: { V: 0, X: 0, Z: 0, L: 0, P: 0 },
      missingActual: 0,
    };
    for (const s of [...slot.shifts].sort((a, b) =>
      a.date.localeCompare(b.date),
    )) {
      const planned: ShiftCell = {
        start_time: t5(s.start_time) || null,
        end_time: t5(s.end_time) || null,
        code: s.code,
      };
      const hasActual = !!(s.actual_start_time && s.actual_end_time);
      const actual: ShiftCell | null = hasActual
        ? {
            start_time: t5(s.actual_start_time),
            end_time: t5(s.actual_end_time),
            code: null,
          }
        : null;
      ...
      days.push({
        ...
        plannedEnd: planned.end_time ?? "",
        plannedHours: h(pm),
        actualStart: actual?.start_time ?? "",
        actualEnd: actual?.end_time ?? "",
        actualHours: h(am),
        source: hasActual ? "actual" : "planned",
        eveningHours: tot.eveningHours,
        nightHours: tot.nightHours,
        sundayHours: tot.sundayHours,
        note: s.actual_note ?? "",
      });
    }
...
const PH = {
  en: {
    ...
    ae: "Tot. loppu",
    ah: "Tot. h",
    basis: "Laskentaperuste",
    eve: "Ilta h",
    night: "Yö h",
    sun: "Su/pyhä h",
    note: "Huom",
    missing: "Toteuma puuttuu (vuoroa)",
    actual: "toteutunut",
    planned: "suunniteltu",
  },
  sv: {
...
export const payrollDayValues = (
  ...
  r.actualEnd,
  r.actualHours,
  r.source === "actual" ? PH[lang].actual : PH[lang].planned,
  r.eveningHours,
  r.nightHours,
  r.sundayHours,
  r.note,
...
export const payrollSummaryValues = (
  r: PayrollSummaryRow,
): (string | number)[] => [
  r.worker,
  r.role,
  ...
  rows
    .map((r) =>
      r
        .map(
          (c) =>
            `"${typeof c === "number" ? String(c) : sanitizeCsvCell(String(c))}"`,
        )
        .join(";"),
    )
    .join("\r\n");
