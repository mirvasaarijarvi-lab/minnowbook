...
const pinnedWidth = (container: HTMLElement): number =>
  Array.from(
    container.querySelectorAll<HTMLElement>("thead th[data-shift-pinned]"),
  ).reduce((total, cell) => total + cell.getBoundingClientRect().width, 0);

export const revealShiftField = (
  field: HTMLElement,
  ...
  if (fieldRect.right > rightEdge)
    container.scrollLeft += fieldRect.right - rightEdge;
  else if (fieldRect.left < leftEdge)
    container.scrollLeft -= leftEdge - fieldRect.left;
};

export const focusAdjacentShiftField = (
  current: HTMLInputElement,
  backwards: boolean,
): boolean => {
  const container = current.closest<HTMLElement>(
    "[data-testid='shift-table-scroll']",
  );
  if (!container) return false;

  const fields = Array.from(
    container.querySelectorAll<HTMLInputElement>(SHIFT_FIELD_SELECTOR),
  );
...
export const focusShiftFieldByArrow = (
  current: HTMLInputElement,
  key: string,
): boolean => {
  const container = current.closest<HTMLElement>(
  ...
  if (key === "ArrowLeft" || key === "ArrowRight") {
    ...
    const col = Array.from(
      row.querySelectorAll<HTMLInputElement>(SHIFT_FIELD_SELECTOR),
    ).indexOf(current);
    const rows = Array.from(container.querySelectorAll("tr")).filter((r) =>
      r.querySelector(SHIFT_FIELD_SELECTOR),
    );
    const r = rows.indexOf(row);
    const target = rows[r + (key === "ArrowUp" ? -1 : 1)];
    next =
      target?.querySelectorAll<HTMLInputElement>(SHIFT_FIELD_SELECTOR)[col];
  } else return false;
