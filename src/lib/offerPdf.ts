...
const L: Record<string, Record<string, string>> = {
  ...
  validity: {
    fi: "Tarjouksen voimassaoloaika:",
    en: "Offer validity:",
    sv: "Erbjudandets giltighetstid:",
  },
  customer_name: {
    fi: "Asiakkaan nimi:",
    en: "Customer name:",
  ...
  linked: {
    fi: "Yhdistetyt varaukset:",
    en: "Linked reservations:",
    sv: "Länkade bokningar:",
  },
  invoicing: {
    fi: "Laskutustiedot:",
    en: "Invoicing details:",
    sv: "Faktureringsuppgifter:",
  },
...
const BG_BAND = rgb(0.965, 0.955, 0.94);
...
export async function generateOfferPdf(
  ...
  const dateStr = format(parseISO(offer.event_date), "d.M.yyyy", {
    locale: dateLocale,
  });
  const timeStr = offer.end_time
  ...
  if (logoImage) {
    ...
    page.drawText(bName, {
      x: ML + dims.width + 12,
      y: y - dims.height / 2 - SECTION_SIZE / 3,
      size: SECTION_SIZE,
      font: boldFont,
      color: TEXT_CLR,
    });
  ...
  if (offer.validity_date) {
    page.drawText(t("validity", lang) + " " + offer.validity_date, {
      x: ML,
      y,
      size: SMALL_SIZE,
      font: bodyFont,
  ...
  if (offer.guest_email)
    custEntries.push([t("customer_email", lang), offer.guest_email]);
  if (offer.guest_phone)
    custEntries.push([t("customer_phone", lang), offer.guest_phone]);
  ...
  if (offer.event_type)
    detailPairs.push([t("event_type", lang), offer.event_type]);
  if (offer.invoicing_details)
    detailPairs.push([t("invoicing", lang), offer.invoicing_details]);
  ...
  if (offer.special_requests && y > MB + 120) {
    ...
    drawDivider(page, y);
    y -= sectionLh;
    page.drawText(t("special_requests", lang), {
      x: ML,
      y,
      size: SECTION_SIZE,
      font: boldFont,
  ...
  const sigY = MB + 80;
  page.drawLine({
    start: { x: ML, y: sigY + 14 },
    end: { x: PW - MR, y: sigY + 14 },
    thickness: 0.5,
    color: DIVIDER_CLR,
  });
  page.drawText(t("greeting", lang), {
    x: ML,
    y: sigY - 4,
    size: BODY_SIZE,
    font: bodyFont,
