...
import { computeChannelSplitByType, buildTrendBuckets, type ChannelRow } from "@/lib/bookingChannelStats";
...
const BookingChannelPanel = () => {
  ...
  const splits = useMemo(() => computeChannelSplitByType(rows, allowedTypes), [rows, allowedTypes]);
  const trend = useMemo(
    () =>
      buildTrendBuckets({
        rows,
  ...
  const handlePdf = () => {
    downloadReportPdf({
      ...
      kpis: [
        ...
        { label: t("an.channel.public"), value: `${splits.all.publicCount} (${splits.all.publicPct}%)` },
        { label: t("an.channel.staff"), value: `${splits.all.staffCount} (${splits.all.staffPct}%)` },
      ],
      chart: {
        ...
        series: [
          { key: "publicCount", label: t("an.channel.public"), color: [37, 99, 235] },
          { key: "staffCount", label: t("an.channel.staff"), color: [148, 163, 184] },
        ],
      },
      table: {
        head: [t("an.service"), t("an.channel.total"), t("an.channel.public"), t("an.channel.staff")],
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <Button variant="outline" size="sm" onClick={handlePdf} disabled={splits.all.total === 0}>
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                { label: t("an.channel.total"), value: String(splits.all.total), sub: "" },
                { label: t("an.channel.public"), value: String(splits.all.publicCount), sub: `${splits.all.publicPct}%` },
                { label: t("an.channel.staff"), value: String(splits.all.staffCount), sub: `${splits.all.staffPct}%` },
              ].map((kpi) => (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={trend} margin={{ top: 5, right: 10, left: -18, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                <XAxis dataKey="label" tick={{ fontSize: 11 }} />
                <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
                <Tooltip />
                <Legend />
                <Bar dataKey="publicCount" name={t("an.channel.public")} stackId="a" fill="hsl(var(--primary))" />
                <Bar dataKey="staffCount" name={t("an.channel.staff")} stackId="a" fill="hsl(var(--muted-foreground))" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">{t("an.channel.total")}</TableHead>
                    <TableHead className="text-right">{t("an.channel.public")}</TableHead>
                    <TableHead className="text-right">{t("an.channel.staff")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {allowedTypes.map((tp) => (
                    <TableRow key={tp}>
                      ...
                      <TableCell className="text-right">{splits[tp]?.total ?? 0}</TableCell>
