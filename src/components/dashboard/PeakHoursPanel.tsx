...
const PeakHoursPanel = () => {
  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("reservations")
        .select(
          "id, reservation_type, date, start_time, guests_count, estimated_guests, site_id",
        )
  ...
  const { chartData, busiest, total } = useMemo(() => {
    const filtered = rows.filter(
      (r) =>
        (service === "all" || r.reservation_type === service) && !!r.start_time,
    );
    ...
    for (const r of filtered) {
      ...
      const value =
        metric === "guests" ? (r.guests_count ?? r.estimated_guests ?? 0) : 1;
    ...
    const top = data.reduce((best, d) => (d.value > best.value ? d : best), {
      hour: -1,
      label: "-",
      value: 0,
    });
