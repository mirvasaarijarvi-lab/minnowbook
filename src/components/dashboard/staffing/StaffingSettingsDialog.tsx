...
export default function StaffingSettingsDialog({
  open,
  onOpenChange,
  lang,
}: {
  ...
  const submit = () =>
    save.mutate(normalizeStaffingSettings(s), {
      onSuccess: () => {
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {types.map((t) => (
              <div key={t} className="space-y-1">
                <Label htmlFor={`gps-${t}`} className="text-xs">
                  {tD(`dashboard.${t}`)}
                </Label>
                <Input
                  id={`gps-${t}`}
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
              <Label htmlFor="min-staff" className="text-xs">
                {L.minStaff}
              </Label>
              <Input
                id="min-staff"
                ...
                value={s.minStaff}
                onChange={(e) =>
                  setS({ ...s, minStaff: Number(e.target.value) })
                }
              />
            </div>
            <div className="space-y-1">
              ...
              <Input
                ...
                min={0}
                max={1000}
                value={s.hourlyCostEur}
                onChange={(e) =>
                  setS({ ...s, hourlyCostEur: Number(e.target.value) })
                }
              />
            </div>
          </div>
            ...
            {(["eveningStart", "eveningEnd", "nightEnd"] as const).map((k) => (
              <div key={k} className="space-y-1">
                ...
                <Input
                  id={k}
                  type="time"
                  value={minutesToHHMM(s.rules[k])}
                  onChange={(e) =>
                    setS({
                      ...s,
                      rules: {
                        ...s.rules,
                        [k]: hhmmToMinutes(e.target.value, s.rules[k]),
                      },
                    })
                  }
                />
          ...
          <label className="flex items-center gap-2 text-sm">
            <Checkbox
              checked={s.rules.finnishHolidays !== false}
              onCheckedChange={(v) =>
                setS({ ...s, rules: { ...s.rules, finnishHolidays: !!v } })
              }
            />
            {L.finnishHolidays}
          </label>
