...
export default function StaffingNeedsPanel({ lang }: { lang: StaffLang }) {
  ...
  const { data: bookings = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("reservations")
        .select(
          "id, reservation_type, start_time, end_time, guests_count, estimated_guests, status",
        )
  ...
  const rosterTimes = useMemo(
    () =>
      roster
        .filter((r) => !r.code)
        .map((r) => ({
          start_time:
            (r.actual_start_time ?? r.start_time)?.slice(0, 5) ?? null,
  ...
  const hours = useMemo(
    () => hourlyNeeds(needBookings, rosterTimes, settings),
    [needBookings, rosterTimes, settings],
  );
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-end gap-3">
        <div className="space-y-1">
          <label htmlFor="needs-date" className="text-xs text-muted-foreground">
            {L.day}
          </label>
          <Input
            id="needs-date"
        ...
        <div className="text-sm">
          {L.estCost}:{" "}
          <strong>{Math.round(staffHours * settings.hourlyCostEur)} EUR</strong>
        </div>
        {under.length > 0 && (
          <Badge variant="destructive">
            {L.understaffedHours}: {under.join(", ")}
          </Badge>
        )}
      </div>
      {rosterTimes.length === 0 && needBookings.length > 0 && (
        <p className="text-xs text-muted-foreground">{L.noList}</p>
      )}
      {active.length === 0 ? (
        <p className="text-sm text-muted-foreground">{L.noBookings}</p>
      ) : (
        <table className="w-full max-w-2xl text-sm">
          ...
          <tbody>
            {active.map((h) => (
              <tr
                key={h.hour}
                className={`border-t border-border ${h.understaffed ? "bg-destructive/10" : ""}`}
              >
                ...
                <td aria-hidden>
                  <div
                    ...
                    style={{ width: `${(h.needed / max) * 100}%` }}
                  />
                  <div
                    className="mt-0.5 h-2 rounded bg-muted-foreground/50"
                    style={{ width: `${(h.rostered / max) * 100}%` }}
                  />
                </td>
              </tr>
            ))}
      ...
      <ul className="text-xs text-muted-foreground">
        {needBookings.map((b, i) => (
          <li key={i}>
            {b.start_time}
            {b.end_time ? ` → ${b.end_time}` : ""}: {b.guests}{" "}
            {L.guests.toLowerCase()}, {L.needed.toLowerCase()}{" "}
            {suggestStaff(b.reservation_type, b.guests, settings)}
          </li>
        ))}
      </ul>
    </div>
