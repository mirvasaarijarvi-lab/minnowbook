    ...
    queryFn: async (): Promise<HealthCheck[]> => {
        ...
        if (error) {
          ...
          results.push({
            ...
            message: data.session
              ? `Active session (${latency}ms)`
              : `Service reachable (${latency}ms)`,
            latency,
          });
        }
      } catch (e: any) {
      ...
      try {
        ...
        if (error) {
          results.push({
            name: "Email System",
            status: "warning",
            message: "Cannot read email logs",
          });
        } else {
          ...
          if (total === 0) {
            ...
            results.push({
              name: "Email System",
              status: "ok",
              message: `${total} emails sent successfully`,
            });
          }
        }
      } catch {
      ...
      try {
        ...
        results.push({
          ...
          message: error
            ? `Responded with error (${latency}ms)`
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            ...
            {checks && !isFetching && (
              <Badge
                variant="outline"
                className={`text-xs ${statusBadge(overallStatus)}`}
              >
                {overallStatus === "ok"
