...
const ReservationList = ({ initialStatusFilter, initialInvoicedFilter, initialCheckoutToday }: ReservationListProps) => {
  ...
  const t = useT();
  const { showRefusal, clearRefusal, formatInvoiceRefusal } = useInvoiceRefusalNotice();
  ...
  const toggleInvoiced = useMutation({
    ...
    onError: (err: unknown) => {
      ...
      // instead of one generic sentence for all of them. Replaces any earlier
      // refusal so only the current one is visible.
      showRefusal(err);
    },
  });
  ...
  const handleToggleInvoiced = async (id: string, checked: boolean) => {
    ...
    if (checked) {
      ...
      if (!hasPrice) {
        showRefusal("Add a price before marking this reservation as invoiced.");
        return;
      }
    }
