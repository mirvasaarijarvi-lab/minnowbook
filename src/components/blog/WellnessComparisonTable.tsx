...
const featureRows: Row[] = [
  {
    label: "Starting monthly price (EUR, VAT incl.)",
    mimmobook: "19",
    mindbody: "~139",
    vagaro: "~28",
    fresha: "0 + fees",
  ...
  {
    label: "Transaction fees on card payments",
    mimmobook: false,
    mindbody: false,
    vagaro: false,
    fresha: true,
  ...
  {
    label: "Team and role management",
    mimmobook: true,
    mindbody: true,
    vagaro: true,
    fresha: true,
  ...
  {
    label: "No marketplace listing required",
    mimmobook: true,
    mindbody: true,
    vagaro: true,
    fresha: false,
...
const WellnessComparisonTable = () => {
  return (
    <section
      aria-labelledby="wellness-comparison-heading"
      className="mb-10 not-prose"
    >
      <h2
        id="wellness-comparison-heading"
        className="font-serif text-2xl font-bold text-foreground mb-4"
      >
        Features and pricing at a glance
      </h2>
      <p className="text-sm text-muted-foreground mb-4">
        Snapshot of typical entry plans across common wellness booking
        platforms. Pricing is indicative and may vary by region and add-ons.
      </p>
        ...
        <table className="w-full text-sm">
          <thead className="bg-secondary/50">
            <tr>
              <th
                scope="col"
                className="text-left font-semibold text-foreground px-4 py-3"
              >
                Feature
              </th>
              {columns.map((c) => (
          ...
          <tbody>
            {featureRows.map((row, i) => (
              ...
              >
                <th
                  scope="row"
                  className="text-left font-medium text-foreground px-4 py-3 align-top"
                >
                  {row.label}
                </th>
                {columns.map((c) => (
      ...
      <p className="text-xs text-muted-foreground mt-3">
        Sources: public pricing pages of each provider, checked at time of
        writing. Always confirm current pricing on the vendor site.
      </p>
    </section>
  );
};
