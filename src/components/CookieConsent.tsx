  ...
  function CookieConsent(_props, ref) {
    ...
    useEffect(() => {
      const open = () => {
        const stored = readConsent();
        setAnalytics(
          stored?.categories.analytics ?? DEFAULT_CATEGORIES.analytics,
        );
        setMarketing(
          stored?.categories.marketing ?? DEFAULT_CATEGORIES.marketing,
        );
        setShowDetails(true);
        setVisible(true);
      };
      ...
      return () =>
        window.removeEventListener("mimmobook:open-cookie-settings", open);
    }, []);
