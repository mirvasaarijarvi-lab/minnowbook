    ...
    <div ref={ref} className="fixed bottom-4 left-4 md:bottom-6 md:left-6 z-[90]">
      {open && (
        <div className="absolute bottom-14 left-0 bg-card border border-border rounded-xl shadow-lg p-4 w-60 space-y-2.5 max-h-[70vh] overflow-y-auto animate-fade-in">
          ...
          <ToggleRow icon={Eye} label={t("a11y.highContrast")} active={prefs.highContrast} onToggle={() => update({ highContrast: !prefs.highContrast })} />
          <ToggleRow icon={Type} label={t("a11y.dyslexiaFont")} active={prefs.dyslexiaFont} onToggle={() => update({ dyslexiaFont: !prefs.dyslexiaFont })} />
          <ToggleRow icon={Pause} label={t("a11y.reducedMotion")} active={prefs.reducedMotion} onToggle={() => update({ reducedMotion: !prefs.reducedMotion })} />
          <ToggleRow icon={MousePointer} label={t("a11y.focusHighlight")} active={prefs.focusHighlight} onToggle={() => update({ focusHighlight: !prefs.focusHighlight })} />

          <Button variant="ghost" size="sm" className="w-full h-7 text-xs gap-1.5" onClick={reset}>
            <RotateCcw className="h-3 w-3" /> {t("a11y.resetAll")}
          </Button>
        </div>
      )}
