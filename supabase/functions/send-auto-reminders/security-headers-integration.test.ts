// Integration test: every Response returned by `send-auto-reminders` carries the
...
import { handleSendAutoRemindersRequest } from "./index.ts";
import {
  assertCspAndHsts,
  assertSharedHeaders,
  drainBody,
...
Deno.test(
  "send-auto-reminders: OPTIONS preflight carries SECURITY_HEADERS",
  withStubSupabaseEnv(async () => {
    const req = new Request("https://example.test/send-auto-reminders", {
      method: "OPTIONS",
      headers: { Origin: "https://mimmobook.com" },
    });
    const res = await handleSendAutoRemindersRequest(req);
    await drainBody(res);
