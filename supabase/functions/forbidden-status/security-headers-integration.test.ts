// Integration test: every Response returned by `forbidden-status` carries the
...
import { handleForbiddenStatusRequest } from "./index.ts";
import {
  assertCspAndHsts,
  assertSharedHeaders,
  drainBody,
...
Deno.test(
  "forbidden-status: OPTIONS preflight carries SECURITY_HEADERS",
  withStubSupabaseEnv(async () => {
    const req = new Request("https://example.test/forbidden-status", {
      method: "OPTIONS",
      headers: { Origin: "https://mimmobook.com" },
    });
    const res = await handleForbiddenStatusRequest(req);
    await drainBody(res);
