// Integration test: every Response returned by `check-subscription` carries the
...
import { handleCheckSubscriptionRequest } from "./index.ts";
import {
  assertCspAndHsts,
  assertSharedHeaders,
  drainBody,
...
Deno.test(
  "check-subscription: OPTIONS preflight carries SECURITY_HEADERS",
  withStubSupabaseEnv(async () => {
    const req = new Request("https://example.test/check-subscription", {
      method: "OPTIONS",
