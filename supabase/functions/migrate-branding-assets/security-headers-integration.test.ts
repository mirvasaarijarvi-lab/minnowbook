// Integration test: every Response returned by `migrate-branding-assets` carries the
...
import { handleMigrateBrandingAssetsRequest } from "./index.ts";
import {
  assertCspAndHsts,
  assertSharedHeaders,
  drainBody,
...
Deno.test(
  "migrate-branding-assets: OPTIONS preflight carries SECURITY_HEADERS",
  withStubSupabaseEnv(async () => {
    const req = new Request("https://example.test/migrate-branding-assets", {
      method: "OPTIONS",
      headers: { Origin: "https://mimmobook.com" },
    });
    const res = await handleMigrateBrandingAssetsRequest(req);
    await drainBody(res);
