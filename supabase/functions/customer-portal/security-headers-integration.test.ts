// Integration test: every Response returned by `customer-portal` carries the
...
import { handleCustomerPortalRequest } from "./index.ts";
import {
  assertCspAndHsts,
  assertSharedHeaders,
  drainBody,
...
Deno.test(
  "customer-portal: OPTIONS preflight carries SECURITY_HEADERS",
  withStubSupabaseEnv(async () => {
    const req = new Request("https://example.test/customer-portal", {
      method: "OPTIONS",
      headers: { Origin: "https://mimmobook.com" },
    });
    const res = await handleCustomerPortalRequest(req);
    await drainBody(res);
