// Integration test: every Response returned by `create-checkout` carries the
...
Deno.test(
  "create-checkout: OPTIONS preflight carries SECURITY_HEADERS",
  withStubSupabaseEnv(async () => {
    const req = new Request("https://example.test/create-checkout", {
      method: "OPTIONS",
      headers: { Origin: "https://mimmobook.com" },
    });
    const res = await handleCreateCheckoutRequest(req);
    await drainBody(res);
