// Integration test: every Response returned by `send-offer-email` carries the
...
import { handleSendOfferEmailRequest } from "./index.ts";
import {
  assertCspAndHsts,
  assertSharedHeaders,
  drainBody,
...
Deno.test(
  "send-offer-email: OPTIONS preflight carries SECURITY_HEADERS",
  withStubSupabaseEnv(async () => {
    const req = new Request("https://example.test/send-offer-email", {
      method: "OPTIONS",
      headers: { Origin: "https://mimmobook.com" },
    });
    const res = await handleSendOfferEmailRequest(req);
    await drainBody(res);
