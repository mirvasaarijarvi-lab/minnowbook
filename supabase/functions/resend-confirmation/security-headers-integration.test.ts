// Integration test: every Response returned by `resend-confirmation` carries the
...
import { handleResendConfirmationRequest } from "./index.ts";
import {
  assertCspAndHsts,
  assertSharedHeaders,
  drainBody,
...
Deno.test(
  "resend-confirmation: OPTIONS preflight carries SECURITY_HEADERS",
  withStubSupabaseEnv(async () => {
    const req = new Request("https://example.test/resend-confirmation", {
      method: "OPTIONS",
      headers: { Origin: "https://mimmobook.com" },
    });
    const res = await handleResendConfirmationRequest(req);
    await drainBody(res);
